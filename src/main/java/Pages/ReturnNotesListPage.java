...
public class ReturnNotesListPage extends UtilitiesMethods {

    private WebDriver driver;

    public ReturnNotesListPage(WebDriver driver) {this.driver = driver;}
    ...
    /*************************************************************************************************************************************/
    public ReturnNotePage open_new_return_note() {
        js = (JavascriptExecutor) driver;
        wait = new WebDriverWait(driver, ofSeconds(300));
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath
                ("//h5[contains(text(),'قائمة مرتجع مبيعات')]")));
